...
def train_wide_resnet(model_name, filename, optimizer_name, criterion_name, scheduler_name, learning_rate, isEarlyStopping): 
    ...
    wide_resnet_model = models.wide_resnet101_2(weights='Wide_ResNet101_2_Weights.DEFAULT')

    # Modify the model for the 3 classes: benign, malignant, normal
    num_features = wide_resnet_model.fc.in_features
    wide_resnet_model.fc = nn.Linear(num_features, 3)
    wide_resnet_model = wide_resnet_model.to(device)
    ...
    # Defining optimizer
    optimizer_helper = OptimizerHelper(model = wide_resnet_model, optimizer_name = optimizer_name, learning_rate = learning_rate)
...
if __name__ == '__main__':
    train_wide_resnet(model_name = "Wide_ResNet_Model", filename = "Wide_ResNet", optimizer_name = "Adam", scheduler_name = "Reduce LR on Plateau", criterion_name = "Cross Entropy Loss Weighted", learning_rate = 0.001, isEarlyStopping = False)