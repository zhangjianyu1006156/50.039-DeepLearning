...
from TrainHelper import data_prep_googlenet, train_and_evaluate, gen_confusion_matrix, view_classification_report, save_log
from plots import plot_accuracy, plot_loss
# from models.LSTMModel import LSTM

def train_googlenet(model_name, filename, optimizer_name, criterion_name, scheduler_name, learning_rate, isEarlyStopping): 
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Obtaining train_dataset, validation_dataset and test_dataset
    train_dataset, validation_dataset, test_dataset = data_prep_googlenet("../data")
    ...
    googlenet_model = models.inception_v3(weights = 'Inception_V3_Weights.DEFAULT')
    ...
    if hasattr(googlenet_model, 'AuxLogits'):  # Check and modify the auxiliary classifier
        googlenet_model.AuxLogits.fc = nn.Linear(googlenet_model.AuxLogits.fc.in_features, num_classes)
    ...
    # Defining optimizer
    optimizer_helper = OptimizerHelper(model = googlenet_model, optimizer_name = optimizer_name, learning_rate = learning_rate)
...
if __name__ == '__main__':
    train_googlenet(model_name = "GoogleNet_Model", filename = "GoogleNet", optimizer_name = "Adam", scheduler_name = "Reduce LR on Plateau", criterion_name = "Cross Entropy Loss Weighted", learning_rate = 0.001, isEarlyStopping = False)